...
def update_env_vars(var_to_update, new_var):
    with open(env_file_path, 'rt') as env_vars_file:
    ...
    if var_to_update in env_vars_contents:
        ...
        with open(env_file_path, 'wt') as env_vars_file:
...
if new_data:
    new_data["issue_link"] = f"https://github.com/{github_repository}/issues/{issue_number}"
